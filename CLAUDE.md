...
- `test-metronome.mjs` — Playwright 자동 검증 (타이밍 정확도 + 기능 17개 체크 + 스크린샷)
...
3. **재동기화 가드**: 스케줄러의 `nextTime < now - 0.12 → 리셋`은 탭 동결 복귀 시 클릭 연타 방지용. 유지.
   **첫 재생 예열**: `waitForClock()`+`waitForSteadyClock()` 대기 + 첫 클릭 0.35s 오프셋(`warmedUp` 변수)은
   iOS에서 resume() 직후 ctx.currentTime이 실시간보다 빨리 질주하는 동안 클릭이 연타로 쏟아지는 것 방지용.
   가드의 연속 발동 감지(`lastResync`, 400ms 내 재발동 시 예약 생략)도 같은 목적. 모두 유지.
   iOS에서도 백그라운드 재생 경로(2번)는 실제로 동작하므로 유지 — iOS 직결 라우팅으로 바꾸는 것 금지.
...
수정 후 `node test-metronome.mjs` 실행 — 17개 체크 전부 PASS여야 하고,
...
- 배포할 때마다 index.html 상단의 `APP_VER`를 올릴 것 — 헤더에 표시되고(기기에서 반영 확인용)
  SW 등록 쿼리(`sw.js?v=...`)로 전달되어 캐시 버전이 자동으로 바뀐다. sw.js는 건드릴 필요 없음.
