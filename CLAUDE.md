...
3. **재동기화 가드**: 스케줄러의 `nextTime < now - 0.12 → 리셋`은 탭 동결 복귀 시 클릭 연타 방지용. 유지.
   **첫 재생 예열**: `waitForClock()`+`waitForLiveEdge()` 대기 + 첫 클릭 0.35s 오프셋(`warmedUp` 변수)은
   iOS에서 오디오 세션이 늦게 열리고 <audio>(MediaStream) 출력이 시작 직후 밀린 구간을 몰아서
   재생(따라잡기)하는 동안 클릭이 연타로 쏟아지는 것 방지용. iOS에서도 백그라운드 재생 경로(2번)를
   유지하기 위한 우회책이므로 유지. (iOS 직결 라우팅으로 바꾸면 백그라운드 재생이 죽는다 — 금지)
